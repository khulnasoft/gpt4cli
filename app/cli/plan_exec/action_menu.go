...
func handleHotkeyOption(option hotkeyOption, diffs []string, params ExecParams) {
	...
	if option.char == "d" {
		fmt.Println()
		_, err := lib.ExecGpt4cliCommandWithParams([]string{"diffs", "--ui", "--from-tell-menu"}, lib.ExecGpt4cliCommandParams{
		...
		fmt.Println()
		_, err := lib.ExecGpt4cliCommandWithParams([]string{"diffs", "--git"}, lib.ExecGpt4cliCommandParams{
			DisableSuggestions: true,
		})
		if err != nil {
			fmt.Printf("\nError showing diffs: %v\n", err)
		...
		fmt.Print("(a)")
		fmt.Println()
		_, err := lib.ExecGpt4cliCommand([]string{"apply"})
		if err != nil {
		...
		fmt.Println()
		_, err := lib.ExecGpt4cliCommand([]string{"reject"})
		if err != nil {
		...
		_, err := lib.ExecGpt4cliCommand([]string{"apply", "--full"})
		if err != nil {
			fmt.Printf("\nError applying changes: %v\n", err)
		}
		fmt.Println()
