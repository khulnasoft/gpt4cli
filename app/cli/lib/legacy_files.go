...
func MigrateLegacyProjectFile(currentUserId string) {
	...
	// New formats map to a user id so we can handle multiple accounts in the same gpt4cli dir
	projectPath := filepath.Join(fs.Gpt4cliDir, "project.json")
	if _, err := os.Stat(projectPath); err == nil {
		...
		v2Path := filepath.Join(fs.Gpt4cliDir, "projects-v2.json")
...
func MigrateLegacyCurrentPlanFile(currentUserId string) {
	...
	planPath := filepath.Join(fs.HomeGpt4cliDir, CurrentProjectId, "current_plan.json")
	...
	if _, err := os.Stat(planPath); err == nil {
		...
		v2Path := filepath.Join(fs.HomeGpt4cliDir, CurrentProjectId, "current-plans-v2.json")
...
func MigrateLegacyPlanSettingsFile(currentUserId string) {
	...
	settingsPath := filepath.Join(fs.HomeGpt4cliDir, CurrentProjectId, CurrentPlanId, "settings.json")
	if _, err := os.Stat(settingsPath); err == nil {
		...
		v2Path := filepath.Join(fs.HomeGpt4cliDir, CurrentProjectId, CurrentPlanId, "settings-v2.json")
