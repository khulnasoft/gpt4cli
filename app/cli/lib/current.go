...
func resolveProject(mustResolve, shouldCreate bool) {
	...
	// check if projects-v2.json exists in Gpt4cliDir
	path := filepath.Join(fs.Gpt4cliDir, "projects-v2.json")
	_, err := os.Stat(path)
...
func mustInitProject(existingSettings *types.CurrentProjectSettingsByAccount) {
	...
	// write current-plans-v2.json to Gpt4cliHomeDir/[projectId]/current-plans-v2.json
