...
func (fileState *activeBuildStreamFileState) buildStructuredEdits() {
	...
	for len(applyRes.NeedsVerifyReasons) > 0 && numTries < BuildStructuredEditsMaxTries {
		...
		fixedProposedUpdatesXmlString := GetXMLTag(content, "Gpt4cliProposedUpdates", true)
		...
		if fixedProposedUpdatesXmlString == "" {
			...
			if err != nil {
				log.Printf("buildStructuredEdits - error unmarshalling Gpt4cliProposedUpdates xml: %v\n", err)
			}
			...
			fixedDescString := GetXMLTag(content, "Gpt4cliProposedUpdatesExplanation", false)
			...
			if fixedDescString != "" {
				...
				if err != nil {
					log.Printf("buildStructuredEdits - error unmarshalling Gpt4cliProposedUpdatesExplanation xml: %v\n", err)
				}
