...
func GetWholeFilePrompt(filePath, preBuildState, changesFile, changesDesc string) string {
	...
	changesWithLineNums := shared.AddLineNumsWithPrefix(changesFile, "g4c-new-")
...
- You MUST NOT add 'Gpt4cli: removed' comments unless the intention of the plan and the explanation is to remove code. NEVER add 'Gpt4cli: removed' comments to the *proposed updates* unless the *explanation* includes the word 'remove'. The *only* time you can add a 'Gpt4cli: removed' comment is if 1 - you are correcting a mistaken comment that was intended as a removal comment, but used the wrong format—for example, if the 'Gpt4cli: ' prefix was incorrectly omitted. In that case you must fix the comment in the fixed proposed updates. 2 - the explanation explicitly stats that code should be removed, but a necessary 'Gpt4cli: removed' comment is missing from the proposed updates, causing the code that should have been removed to incorrectly remain in the file. In that case you must add the missing 'Gpt4cli: removed' comment to the proposed updates in the correct location, with correct surrounding context and anchors.
`
...
- If the proposed changes explanation is *not* correct, you must rewrite the proposed changes explanation to correctly follow *all* the rules in the [Proposed Updates Rules] section in a <Gpt4cliProposedUpdatesExplanation> element. Include *only* the rewritten proposed changes explanation inside the <Gpt4cliProposedUpdatesExplanation> element, and no other text. If the proposed changes explanation is already correct, skip this step.
  - When rewriting the proposed changes explanation, DO NOT change the alter the *type* of the change. You must ONLY update the portion of the explanation that explains the *location* of the change, not the *type* of the change. For example, if the proposed changes explanation is "I'll add 'someFunction' at the end of the file, immediately after 'existingFunction1'", you must rewrite it to correctly explain the *location* of the change, like "I'll add 'someFunction' at the end of the file, immediately after 'existingFunction2'". You MUST NOT change the type of the change, such as from 'add' to 'remove'.
	...
	- If you are outputting a rewritten proposed changes explanation, YOU ABSOLUTELY MUST DO SO within the <Gpt4cliProposedUpdatesExplanation> element. DO NOT output it outside of the <Gpt4cliProposedUpdatesExplanation> element or anywhere else in your response—ONLY output the rewritten proposed changes explanation inside the <Gpt4cliProposedUpdatesExplanation> element.
...
- In a '## Evaluate Proposed Changes' section, examine the proposed changes and determine whether they are correct. 
	...
	d. Are any *reference comments* or *removal comments* written incorrectly? All reference comments must be *exactly* '... existing code ...' and all removal comments must be *exactly* 'gpt4cli: removed code'. Each must use the correct comment syntax for the language. If any comments are variations of those rather than the exact strings, they are incorrect.
...
- In a <Gpt4cliProposedUpdates> element, output the corrected proposed changes. Include *only* the corrected proposed changes inside the <Gpt4cliProposedUpdates> element, and no other text. The corrected proposed changes MUST follow *all* the rules in the [Proposed Updates Rules] section.
	- If you are outputting a corrected proposed changes, YOU ABSOLUTELY MUST DO SO within the <Gpt4cliProposedUpdates> element. DO NOT output it outside of the <Gpt4cliProposedUpdates> element or anywhere else in your response—ONLY output the corrected proposed changes inside the <Gpt4cliProposedUpdates> element. You ABSOLUTELY MUST NOT use triple backticks or any other formatting around the rewritten proposed changes. Output ONLY the corrected proposed changes inside the <Gpt4cliProposedUpdates> element with no other text or formatting.
	- DO NOT include the proposed changes within a formatted code block. Include *ONLY* the code directly inside the <Gpt4cliProposedUpdates> element.
...
Note that if the changes were applied correctly, YOU ABSOLUTLY MUST NEVER output a <Gpt4cliProposedUpdates> element. Just stop the response after the ## Evaluate Proposed Changes section.
`
	}
...
`
	...
	s += `
<Gpt4cliProposedUpdatesExplanation>
