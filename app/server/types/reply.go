...
func (r *ReplyParser) AddChunk(chunk string, addToTotal bool) {
	...
	if r.maybeFilePath != "" && !r.isInMoveBlock && !r.isInRemoveBlock && !r.isInResetBlock {
		...
		if strings.HasPrefix(prevFullLineTrimmed, "<Gpt4cliBlock") {
			if verboseLogging {
	...
	if r.currentFilePath == "" && !r.isInMoveBlock && !r.isInRemoveBlock && !r.isInResetBlock {
		...
		if LineHasXmlPath(prevFullLineTrimmed) {
			if verboseLogging {
				log.Println("Line has XML-style Gpt4cliBlock tag")
			}
		...
		if prevFullLineTrimmed == "</Gpt4cliBlock>" {
			if verboseLogging {
		...
		if prevFullLineTrimmed == "<EndGpt4cliFileOps/>" {
			if verboseLogging {
...
func LineHasXmlPath(line string) bool {
	return strings.HasPrefix(line, "<Gpt4cliBlock") && strings.Contains(line, `path="`)
}
...
func extractFilePath(line string) string {
	// Handle XML-style Gpt4cliBlock tag
	if strings.HasPrefix(line, "<Gpt4cliBlock") {
