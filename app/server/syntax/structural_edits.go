...
type FixedProposedUpdatesTag struct {
	XMLName xml.Name `xml:"Gpt4cliProposedUpdates"`
	Content string   `xml:",chardata"`
}

type ProposedUpdatesExplanationTag struct {
	XMLName xml.Name `xml:"Gpt4cliProposedUpdatesExplanation"`
	Content string   `xml:",chardata"`
}
...
func isRemoval(content string) bool {
	return strings.Contains(strings.ToLower(content), "gpt4cli: removed")
}
